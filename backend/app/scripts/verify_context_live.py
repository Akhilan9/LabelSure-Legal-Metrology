"""Controlled Phase 7 OCR-to-context loopback HTTP verification."""
import json
import secrets
import socket
import tempfile
...
TEXT = ["APEX Test Masala", "Net Quantity: 500 g", "MRP: ₹120",
...
def main():
    with tempfile.TemporaryDirectory(prefix="labelsure-phase7-") as tmp:
        ...
        try:
            ...
            with httpx.Client(base_url=f"http://127.0.0.1:{port}/api/v1", timeout=30) as client:
                ...
                inspection = client.post("/inspections", json={"product_name":TEXT[0],"category":"Food","package_type":"Pouch","import_status":"DOMESTIC"})
                ...
                client.post(base+"/images/"+image+"/process").raise_for_status()
                ...
                context=client.post(base+"/resolve-context")
                context.raise_for_status()
                context_ms=round((time.perf_counter()-context_started)*1000,2)
                resolved=context.json()
                assert resolved["resolved"]["QUANTITY_KIND"]["value"]=="WEIGHT"
                assert resolved["resolved"]["IMPORT_STATUS"]["value"]=="DOMESTIC"
                assert resolved["resolved"]["PACKAGE_TYPE"]["value"]=="POUCH"
                snapshot=client.get(base+"/rule-input")
                snapshot.raise_for_status()
                assert digest(snapshot.json()["content"])==snapshot.json()["content_sha256"]
                preview=client.get(base+"/applicability-preview")
                preview.raise_for_status()
                assert all(not item["executable"] for item in preview.json()["items"])
                assert client.post(base+"/resolve-context").json()["run"]["id"]==resolved["run"]["id"]
                assert client.get(base+"/rule-input").json()==snapshot.json()
                assert all('"'+word+'"' not in json.dumps(snapshot.json()) for word in ["PASS","FAIL","COMPLIANT","NON_COMPLIANT"])
                print(json.dumps({"transport":"loopback HTTP","ocr":"controlled deterministic fixture",
                    "candidate_count":len(rows),"context_status":resolved["resolution_status"],
                    "evidence_sufficiency":resolved["evidence"]["state"],
                    "facts_created":resolved["run"]["facts_created"],
                    "quantity_kind":resolved["resolved"]["QUANTITY_KIND"]["value"],
                    "import_status":resolved["resolved"]["IMPORT_STATUS"]["value"],
                    "snapshot_integrity":True,"idempotency":True,"legal_rules_executed":False,
                    "context_http_ms":context_ms},indent=2))
